...
public class OnlineGameController {
    ...
    public void initialize() {
        ...
        try {
            ...
            System.err.println("Impossible de charger l'image du coeur : " + e.getMessage());
        }
        ...
        try {
            ...
            System.err.println("Impossible de charger l'image de la bombe personnalisee");
        }
        ...
        nm.addServerMessageListener(MessageType.GAME_UPDATE, msg -> {
            if (msg instanceof JoueurMisAJourDTO update) {
                Platform.runLater(() -> {
                    ...
                    boolean anyRemoteAlive = remotePlayers.values().stream().anyMatch(Joueur::isAlive);
        ...
        nm.addServerMessageListener(MessageType.BOMB_UPDATE, msg -> {
            if (msg instanceof BombUpdate update) {
                Platform.runLater(() -> {
                    ...
                    if (update.getLabyrinthe() != null) this.labyrinthe = update.getLabyrinthe();
                });
            }
        });
    ...
    private void handleInputs() {
        ...
        double dx = 0, dy = 0;
        if (pressedKeys.contains(KeyCode.Z) || pressedKeys.contains(KeyCode.UP))    dy--;
        if (pressedKeys.contains(KeyCode.S) || pressedKeys.contains(KeyCode.DOWN))  dy++;
        if (pressedKeys.contains(KeyCode.Q) || pressedKeys.contains(KeyCode.LEFT))  dx--;
        if (pressedKeys.contains(KeyCode.D) || pressedKeys.contains(KeyCode.RIGHT)) dx++;
        ...
        if (pressedKeys.contains(KeyCode.SPACE) && !spaceWasPressed) {
    ...
    private void update(double deltaTime) {
        if (pressedKeys.contains(KeyCode.ESCAPE) && !escWasPressed && gameCanvas.isFocused()) {
            escWasPressed = true;
        ...
        if (isVictory || isGameOver) {
            if (isGameOver && deathAnimationStartTime > 0) {
                if (System.currentTimeMillis() - deathAnimationStartTime >= DEATH_ANIMATION_DURATION)
        ...
        if (localPlayer.isAlive() && !isVictory) handleInputs();
    }
    ...
    private void render() {
        ...
        if (localPlayer.getId() != -1 && localPlayer.isAlive()) renderer.drawPlayer(gc, localPlayer);
        for (Joueur remote : remotePlayers.values()) {
            if (remote.getId() != localPlayer.getId() && remote.isAlive()) renderer.drawPlayer(gc, remote);
        }

        if (localPlayer.isAlive()) drawStatsBar(gc, localPlayer.getNb_bombes(), localPlayer.getPv(), localPlayer.getExplosionRange(), localPlayer.getSpeed_multiplier());
        if (isVictory) drawVictoryScreen();
        else if (isGameOver && deathAnimationComplete) drawGameOverScreen();
    }
    ...
    private void drawVictoryScreen() {
        ...
        gc.fillText("VICTORY !", gameCanvas.getWidth() / 2 - 120, gameCanvas.getHeight() / 2);
        gc.setFill(javafx.scene.paint.Color.WHITE);
        gc.setFont(javafx.scene.text.Font.font("Arial", 20));
        gc.fillText("ESC pour retourner au menu", gameCanvas.getWidth() / 2 - 140, gameCanvas.getHeight() / 2 + 50);
    }
    ...
    private void drawStatsBar(GraphicsContext gc, int bombs, int hearts, int range, float speed) {
        double barHeight = 35, barY = gameCanvas.getHeight() - barHeight - 5, barWidth = 320;
